...
#
# Descriptor-probing logic (2026-06-29, ported from upstream a063aad's
# sibling commit 0b465ad "broaden descriptor probes for null-field cases"):
# expanded attribute candidate sets + locale-aware callable invocation, since
# different SPs / device-package authors expose name/vendor/category under
# different property names (and some are localised getters that need a
# locale arg). Purely additive robustness on a read-only tool -- no online
# dependency, safe to adopt without on-PLC verification.
